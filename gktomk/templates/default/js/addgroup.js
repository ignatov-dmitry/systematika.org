            ...
            $.ajax({
                ...
                success: function (response) {
                    ...
                    addgroup.data['programs'].forEach(program => {
                        ...
                        links += '<li class="nav-item"><a href="#" onclick="addgroup.loadPrograms.selectProgram(this);" data-id-program="' + program['id'] + '" class="nav-link ' + active + ' programs-link">' + program['shortname'] + '</a></li>';
                    });
                    $('#programs-links').html(links);
                }
            });
        }
    ...
    loadClasses: {
        ...
        loadLinks: function (idProgram) {
            ...
            addgroup.data['classes_selected'][idProgram].forEach(class_id => {
                num += 1;
                let nameLink = addgroup.data['classes'][class_id]['shortname'];
                let active = '';
    ...
    loadGroups: {
        ...
        loadLinks: function (idProgram, idClass = 0, idManager = 0) {
            ...
            addgroup.data['groups_data'].forEach(group => {
                ...
                if (group['program_id'] !== idProgram || !group['show_adm'])
                    return;
                ...
                daysTimes[beginDay+''+beginMinute].push(['<li class="nav-item"><a href="#" data-id-group="' + group['group_id_mk'] + '" onclick="addgroup.loadGroups.openModal(this);" class="">' + beginTime + '</a> <span style="font-size: 16px;color: '+color+';">&#x25CF;</span>'+comment+'</li>', dayWeek]);
        ...
        openModal: function (el) {
            ...
            $.ajax({
                url: SETT.URL_SITE + '/addgroup/data-group-ajax/' + idGroup + '?email=' + $_GET('email'),
                type: 'GET',
                dataType: 'html',
                success: function (response) {
        ...
        modalContentConstruct: function (data, idContent) {
            ...
            let programText = data['group']['programname']+', '+data['group']['weekdaytime']+', '+data['group']['classname'];
            ...
            if (data['lessons']['0']) {
                ...
                $('[for="dateLessonNearest"]').html('Ближайшее ' + day + ' ' + month);
            }
            ...
            if (data['lessons']['1']) {
                ...
                month = lessonDate.toLocaleString('default', {month: 'short'});
                day = lessonDate.getDate();
                $('[for="dateLessonNext"]').html('Следующее ' + day + ' ' + month);
            }


            UserInfoMK.widgetFunctions.visitstatstable.tableConstruct(data['schedule'], 'lessonstatvisits', 0);
        ...
        sendForm: function () {
            $.ajax({
                url: SETT.URL_SITE + '/addgroup/addgroup-ajax/',
                ...
                success: function (response) { //Данные отправлены успешно
                    ...
                    if (resp.addgroup && resp.addgroup.message) {
                        ...
                        Notify.generate('Пользователь добавлен в группу.', 'Успех!', 1, 6000);
                    ...
                    if (resp.addlesson && resp.addlesson.message) {
                        ...
                        Notify.generate('Пользователь записан на урок.', 'Успех!', 1, 6000);
